...
- `POST /api/v1/qa-routing/recommend`
...
学生端问答入口当前有三类路由判断，边界不要混用：

- `POST /api/v1/course-routing/recommend`：无显式课程时识别“是哪门课”。Java 生成课程画像并委托 GraphRAG Python 内部 embedding 路由，返回 `matched`、`needs_confirmation` 或 `no_match`，用于决定是否自动选课或提示学生确认课程。
- `POST /api/v1/qa-routing/domain-check`：已解析课程或知识库后判断“是否适合进入课程知识库问答”。它不是课程识别接口，也不负责选择 `local` / `global` / `hybrid_v0` 等 QA 检索模式。
- `POST /api/v1/qa-routing/recommend`：只推荐 QA 检索模式。调用前应已经有明确课程、知识库或会话上下文，后端会先校验会话/课程/知识库权限，再给出模式推荐。

课程问答域校验 `domain-check` 的响应采用保守语义：判定值只使用 `allowed` 或 `out_of_scope`，同时返回 `reasonCode`、面向前端展示或日志解释的 `message`，以及固定策略标识 `strategy=rule_domain_guard_v1`。其中：

- `allowed` 表示问题可以继续进入课程知识库问答流程，前端随后可按现有逻辑创建 session、发送消息并启动 task。
- `out_of_scope` 表示问题明显不属于当前课程知识库问答域，前端应阻断正式问答提交。

第一版规则只拦明显越界问题，避免把弱但合理的课程提问误伤。例如 `帮我复习一下` 这类没有明确知识点、但仍可能发生在当前课程语境中的问题，默认放行。强负样本应稳定返回 `out_of_scope`，至少包括：
...
问答范围闸门配置项以 `CKQA_QA_DOMAIN_GUARD_*` 为主，默认启用 `enabled=true`，`outOfScopeThreshold=0.33`。当前阈值小幅高于“什么是二叉树”在操作系统画像上的边界分数（约 0.329692）；“什么是死锁？”这类定义题若命中当前课程画像核心词，会在课程相关性评估中获得最低相关度保底，避免误拦。若后续新增课程画像或更换 embedding 模型，应通过 `QaScopeGateOnlineCalibrationTest` 复校后再调整 `CKQA_QA_DOMAIN_GUARD_OUT_OF_SCOPE_THRESHOLD`。
...
- `POST /api/v1/course-routing/recommend`：学生端没有显式 `courseId` 时调用；Java 生成课程画像并委托 GraphRAG Python 内部 embedding 路由，返回 `matched`、`needs_confirmation` 或 `no_match`。
...
课程画像路由配置项以 `CKQA_COURSE_ROUTING_*` 为主，默认启用 `text-embedding-v4`、`topK=3`、`scoreThreshold=0.30`、`definitionOffTopicScoreThreshold=0.33`、`marginThreshold=0.06`、`embeddingDimensions=1024`，LanceDB 表名默认为 `course_profiles_text_embedding_v4`。普通短概念仍使用 `scoreThreshold=0.30`，避免误伤“什么是线程”这类课程核心问题；对于“什么是 X”定义题，如果 X 没有命中候选课程画像核心词，则使用更严格的 `definitionOffTopicScoreThreshold=0.33`，避免“什么是二叉树”这类非当前课程短概念被自动绑定到操作系统。显式问“二叉树在操作系统中有哪些应用”仍可因问题本身包含操作系统语境而路由到 OS。已选课程后的边界问题仍依赖后续 `domain-check` 与 `CKQA_QA_DOMAIN_GUARD_OUT_OF_SCOPE_THRESHOLD` 共同拦截。后续应继续用课程路由样本集滚动调整。浏览器仍只调用 Java `/api/v1`，不直连 Python `/v1/internal/course-routing/*`。

判定语义：`score < scoreThreshold` 返回 `no_match`，用于食堂、创作、头像等明显非课程问题；`score >= scoreThreshold` 但 `margin < marginThreshold` 返回 `needs_confirmation`，用于“这门课什么时候考试”这类可能属于课程语境但无法确定课程的问题；两个阈值都达标才返回 `matched` 并自动选课。

内部或演示课程不要依赖课程名关键词排除；使用显式配置或课程标签：`CKQA_COURSE_ROUTING_EXCLUDED_COURSE_IDS` 默认包含本地内部 smoke 课程 `crs-20260505-r5z6kq`，`CKQA_COURSE_ROUTING_EXCLUDED_COURSE_TAGS` 默认包含 `course-routing-excluded,internal`。后续新增内部课程时，优先给课程 `tags` 增加 `course-routing-excluded` 或把课程 ID 加入环境变量。

课程画像文本会先汇总课程名、简介、元数据、知识库和资料标题；如果课程存在 active 知识库索引，Java 还会通过 Python 内部 `/v1/internal/course-routing/profile-hints` 从 active build-run 的 `section_docs.json` / `text_units.parquet` 抽取章节来源和关键词 hints。该增强是通用流程，不绑定“操作系统2026春”；新增课程只要完成资料解析、GraphRAG 输入同步、索引构建并激活 build run，就会复用同一套画像增强。Python 不读取 `entities` / `community_reports` 派生产物；hints 抽取失败时 Java 记录 warn 并继续使用基础画像，避免影响问答主链路。

可选开关：

```bash
export CKQA_COURSE_ROUTING_PROFILE_HINTS_ENABLED=true
export CKQA_COURSE_ROUTING_PROFILE_HINTS_MAX_HINTS=24
export CKQA_COURSE_ROUTING_EXCLUDED_COURSE_IDS=crs-20260505-r5z6kq
export CKQA_COURSE_ROUTING_EXCLUDED_COURSE_TAGS=course-routing-excluded,internal
```

## 学生端 QA 任务事件流

真实问答仍采用异步 task 模型：`POST /api/v1/qa-sessions/{id}/messages` 创建用户消息和后台任务，随后可通过 `GET /api/v1/qa-sessions/{sessionId}/tasks/{taskId}` 轮询。P3-C2 提供 SSE 事件流：
...
```bash
export CKQA_QA_STREAM_ENABLED=true
export CKQA_QA_STREAM_TIMEOUT_SECONDS=300
export CKQA_QA_STREAM_STATUS_INTERVAL_SECONDS=2
export CKQA_QA_STREAM_HEARTBEAT_SECONDS=15
export CKQA_QA_STREAM_DELTA_CHARS=120
export CKQA_QA_PYTHON_STREAM_ENABLED=true
export CKQA_QA_PYTHON_STREAM_MODES=basic,local,global,drift,hybrid_v0
export CKQA_QA_PYTHON_STREAM_REPLAY_MAX_CHARS=12000
```
