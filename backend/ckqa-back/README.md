...
课程画像路由配置项以 `CKQA_COURSE_ROUTING_*` 为主，默认启用 `text-embedding-v4`、`topK=3`、`scoreThreshold=0.30`、`marginThreshold=0.06`、`embeddingDimensions=1024`，LanceDB 表名默认为 `course_profiles_text_embedding_v4`。`scoreThreshold=0.30` / `marginThreshold=0.06` 在保留分差约束的前提下，对“什么是线程？”这类短概念问题更友好；明显非课程问题仍依赖分数阈值和后续 `domain-check` 共同拦截。后续应继续用课程路由样本集滚动调整。浏览器仍只调用 Java `/api/v1`，不直连 Python `/v1/internal/course-routing/*`。
