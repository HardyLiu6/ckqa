...
状态：P2 落实后，P3 已按实施方向拆分；智能推荐路由闭环已迁入 main，学生端 Redis 服务端读缓存已接入
...
状态：旁路 PoC 与 D1.2 小步调参对比均已落地；当前结论是不急于接入正式链路。
...
3. 实验端点只支持 `local` query engine，不接入正式 `/v1/query-tasks`，不改变 Java `/api/v1/qa-sessions`。
4. PoC 使用 build-run `dataDirUri` resolver，拒绝绝对路径和路径穿越。
5. 已新增 dry-run 优先的评估脚本 `graphrag_pipeline/scripts/run_graphrag_history_poc_eval.py`，默认输出到 `docs/reports/graphrag-history-poc/`。
6. 实施计划已固化到 `docs/superpowers/plans/2026-05-19-graphrag-query-engine-history-poc.md`。
...
4. 当前不建议直接迁移正式 QA 链路到 query engine history：
   - 即使限制 context token 和 topK，GraphRAG 内部仍出现 `Reached token limit - reverting to previous context state` 警告。
   - LocalSearch history 的来源覆盖没有在小样本中体现稳定收益。
   - 当前 CKQA rewrite/hybrid 链路已经有来源解析、上下文控制、权限和日志闭环，迁移 query engine 需要更多质量收益证据。
5. 如果继续推进，建议只做离线实验和管理端诊断入口，不接学生端正式问答：
   ...
   - 只有当 LocalSearch history 在来源覆盖或追问理解上稳定优于 hybrid baseline，才进入 D1.3 接入设计。
