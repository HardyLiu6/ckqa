...
状态：已完成 PoC、D1.2 小步调参，并接入正式 Python `/v1/query-tasks` 的 Local Beta 路径；学生端正式 QA 仍通过 Java 编排，不直连 Python。
...
状态：已完成首版系统级 Beta 接入，默认关闭，用户显式开启后才参与 `local` 模式上下文。
...
1. 当前学习记忆的生成、晋升和淘汰策略仍偏保守，后续需要结合学生反馈和管理端审核样本优化。
2. 管理端默认不展示学生私密记忆正文，只展示策略和计数；如需教师可见，需要单独做隐私产品决策。
...
5. **P3-D2：长期学习记忆治理**：首版已实现默认关闭的用户开关，后续重点是记忆生成/清理策略、隐私审计和教师可见性边界。
