...
状态：核心正式链路、P0–P2 和 P3-A 已关闭；P3-C2 的 SSE 阶段 1/2、P3-D1 Local history Beta、P3-D2 跨对话学习记忆 Beta 已完成。本文只将尚未交付或尚缺效果证明的事项保留为后续工作。
...
- **已关闭**：已进入正式链路，具备对应的权限、持久化、测试或维护闭环。
- **Beta 已完成**：功能已可用且有回退/隔离边界，但仍限制使用范围，后续只做质量或生产化验证。
- **待实施**：尚未形成可验收闭环，不应表述为已完成。

| 范围 | 归档状态 | 当前结论 |
| --- | --- | --- |
| P0（2 项）/ P1（5 项）/ P2（6 项） | 已关闭 | 正式 QA、权限、会话、来源、缓存和维护能力已完成闭环。 |
| P3-A：反馈与 QA 运维 | 已关闭 | 学生反馈、来源人工复核、运维筛选、详情和样本导出均已落地。 |
| P3-B：Hybrid 质量基础 | 部分完成 | 评测、置信度告警、warmup 降级和离线 rerank 试验基础已具备；`hybrid_v1` 的质量收益仍未证明。 |
| P3-C1：生产保护与缓存治理 | 待实施 | Redis 仅承担低风险读缓存；QA 限流、排队、缓存指标与治理尚未完成。 |
| P3-C2：SSE 任务事件流 | Beta 已完成 | 阶段 1/2 已实现并完成一次成功原生流式 live smoke；多实例事件缓冲仍待生产化。 |
| P3-D1：Local history | Beta 已完成 | `local_history` 已接入正式任务路径并可回退；扩样本效果验证未完成。 |
| P3-D2：跨对话学习记忆 | Beta 已完成 | 偏好、规则式捕获、隔离、注入、读取与清理已完成；质量和隐私产品治理待后续决策。 |
...
P3 不应再被作为单一待办列表。已完成的产品闭环归入本节的归档记录；仍需实现或需要更多证据的工作保留为独立增强项，避免把“功能已可用”和“效果已证明”混为一谈。

### P3-A：QA 反馈闭环与管理端运维面板（已关闭）

已完成范围：

1. 学生端已支持有用、无用、来源不相关、答案过长、希望举例等轻量反馈。
2. `qa_message_feedback` 将反馈关联到 `session/message/retrieval_log`；`qa_source_reviews` 支持教师或管理员复核来源相关性和引用质量。
3. 管理端 QA 运维页已支持按任务状态、mode、反馈、路由置信度和复核优先级筛选；详情页可查看检索问题、上下文、来源、耗时、反馈和人工复核，且可导出样本。
4. 该能力不新增在线 LLM 调用，后续工作只需消费积累的反馈数据，不再重建反馈或运维基础设施。

后续边界：基于真实反馈生成混淆矩阵和周报属于 P3-B 的评测工作，不作为 P3-A 未关闭理由。

### P3-B：hybrid_v1 检索质量与延迟优化（部分完成）
...
2. **不建议默认全量开启 hybrid 自动路由**：虽已有 100 条带 gold text unit 的 QA 样本、152 条路由用例和代表性 live smoke，但尚不足以证明所有课程、题型与索引版本稳定。
...
1. 100 条带专家审定 text unit 标注的 QA 样本、152 条路由用例、版本化 routing smoke 报告和 `--compare-to` 对比已具备。
2. `0.50-0.65` 已标记为 `low_confidence`；学生端可手动切换，管理端可按低置信度和复核优先级筛选。
3. Playwright mock E2E 已覆盖 Beta 开启后以后端推荐为准；warmup 未 ready 时 Smart 路径自动降级并提示。
4. `requestedMode`、`resolvedMode` 和 routing snapshot 已写入检索日志，已具备区分推荐与执行模式的审计基础。
5. 离线评分、baseline/Hybrid 对比和 gated dense rerank 试验工具已具备，但默认不改变正式问答策略。

待实施：

1. 补负样本和边界样本，并基于学生反馈、教师复核和来源标注形成每周混淆矩阵与质量报告。
2. 在同一 holdout 集上验证 rerank/融合候选是否同时改善 citation recall、source precision、rank consistency、延迟和失败率；不达标不得替换 `hybrid_v0` 默认路径。
3. 评估不新增在线 LLM 的轻量语义路由，并按课程类型验证是否确有独立权重需求。
...
新增遗留问题：**表格与图片媒体来源未进入 QA 可展示证据链路**

优先级：P3-B 子项。

现象：

1. 课程材料中包含表格和图片，解析产物也会把图片对象保存到对象存储，但学生端问答结果通常只返回文字答案和文字来源卡片。
2. 表格即使被 MinerU 解析为 HTML/table body，也会在 GraphRAG 中作为普通文本参与检索，最终回答不保证保留原始表格结构。
3. 图片目前主要依赖图题、图注或相邻正文参与检索；图片对象路径存在于解析/导出 metadata 中，但不会自动成为 QA answer 或 source card 的可展示媒体。

当前原因：

1. 当前 GraphRAG 问答链路本质上是文本检索和文本生成，不是多模态检索输出。
2. `pdf_ingest` 的表格渲染器会尽量把表格转成 Markdown/语义文本，但 GraphRAG 生成阶段可能把它摘要化、压缩化或改写为普通段落。
3. `pdf_ingest` 的图片渲染器优先保留图题/图注，不把图片对象引用写入正文；`image_refs/images/tables` 只保存在标准文档 metadata 中。
4. 当前 `graphrag_pipeline/settings.yaml` 只把章节、页码、source_file 等字段作为 GraphRAG ingest metadata；图片和表格 metadata 没有稳定进入 text unit 来源解析链路。
5. `query_citation_resolver.py` 当前只把 `Sources/Reports/Entities/Relationships` 解析成文字来源，没有把命中的 text unit 反查为表格预览、图片缩略图或对象存储 URL。
6. Java `/api/v1` 的 QA source response 和 student-app 来源卡片还没有媒体字段契约；浏览器不会直接访问 Python 或 MinIO 内部路径。

建议下一步：

1. 在 `normalized_docs.json` / GraphRAG 投影中为表格和图片生成稳定 `media_manifest`：
   - `mediaId`
   - `type=image|table`
   - `sourceFile`
   - `page`
   ...
   - `textDescription`
   - `objectKey`
   - `publicUrl` 或 Java 可签名访问 ID。
2. 对表格保留两份内容：
   - 可检索文本：用于 GraphRAG 召回和生成；
   - 可展示结构：Markdown table / HTML table / 原始 table metadata，用于来源卡片预览。
3. 对图片补足可检索描述：
   - 优先使用 MinerU 图题、图注、图片 footnote；
   - 可选增加 OCR/视觉摘要，但必须显式配置，避免默认增加模型成本。
4. 扩展 citation resolver：命中 text unit 后，根据 `block_ids/page/media_manifest` 反查关联表格和图片，输出 `mediaSources`。
5. 扩展 Java `/api/v1` QA source DTO：增加媒体来源字段，仍由 Java 负责鉴权和对象访问签名，不让 student-app 直连 Python `/v1` 或内部 MinIO key。
6. student-app 来源卡片增加：
   - 表格预览；
   - 图片缩略图；
   - “查看原图 / 定位到材料页”入口；
   - 媒体来源不相关反馈标签。
7. 补评测样本：增加“请解释图 X”“根据表格比较”“引用图示说明流程”“结合表格数据计算”等题型，分别覆盖 `local/global/hybrid_v0`。

验收重点：

1. 表格类问题能返回至少一个可展示表格来源，且正文来源编号能和来源卡片对应。
2. 图片类问题能返回图片缩略图或图注来源，且通过 Java `/api/v1` 鉴权访问。
3. 纯文字问题不被无关图片/表格来源污染。
4. 对象存储路径不直接暴露给前端；前端只使用 Java 返回的安全访问 URL 或媒体 ID。
5. 缺少图片文件、表格 HTML 解析失败或对象存储不可用时，QA 主流程仍降级为文字来源，不中断回答。

### P3-C1：生产保护、限流与缓存治理（待实施）
...
当前缺口：尚无面向 QA 任务的用户级、课程级、知识库级或 mode 级并发控制；`StudentRedisCacheService` 也尚未提供缓存命中、异常、失效耗时和 value 大小等治理指标。
...
状态：**阶段 1/2 已完成 Beta 闭环**。Java SSE、Python GraphRAG 原生 streaming 桥接、前端 delta 渲染与 polling 降级均已实现；`20260520-230912-native-streaming-java-sse` live smoke 已成功收到 110 个 delta、首个 delta 约 8.2 秒、最终 5 条来源。该项不是新的 QA mode。

待实施仅保留阶段 3：构建可跨实例工作的短期事件缓冲、标准化断线重连/有限历史回放，并在真实代理环境完成长任务压测。
...
### P3-D：GraphRAG conversation history 与长期学习记忆 Beta（功能已完成，持续评估）
...
P3-A、SSE 阶段 1/2 和两项记忆 Beta 已完成，不应再占用下一轮实施顺序。建议按风险和依赖关系推进：

1. **P3-C1 最小生产护栏**：先以 Java 单实例配置实现用户/课程/知识库/mode 并发上限、明确 429/排队反馈和缓存基础指标。它不改变 QA 主协议，却能限制现有 Beta 的资源风险。
2. **P3-B 评测与反馈数据闭环**：从已有反馈、来源复核和运维导出生成混淆矩阵，补齐 holdout/负样本；先获得 v0 基线，再评估 rerank 和融合候选。
3. **P3-B `hybrid_v1` 实验性优化**：仅在离线指标和 live smoke 都优于 v0 时，以 Beta/配置开关灰度；未达标则保留 v0 默认策略。
4. **P3-C2 阶段 3 生产化**：在任务限流边界明确后，补事件缓冲、重连回放、代理配置和多实例压测；保持 polling 作为最终降级路径。
5. **P3-B 媒体来源链路**：作为独立跨模块项目，从 `media_manifest`、citation resolver、Java 鉴权 DTO 到学生端预览一次性完成，不与 Hybrid 算法调参混做。
6. **P3-D1 Local history 扩样本评估**：在同一批 20–30 条人工审定追问上比较 CLI local、local history 与 hybrid，确认收益后再扩大课程或模式范围。
7. **P3-D2 长期记忆治理**：最后处理质量评估、学生编辑/合并、教师可见性的隐私决策及可能的语义压缩；在产品边界确定前不引入默认 LLM 抽取。

如果只选择一个最先做，推荐从 **P3-C1 最小生产护栏** 开始；P3-A 已经提供了后续评测所需的数据入口，无需重建。
