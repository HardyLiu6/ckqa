...
- `GET /system/health` 是轻量健康检查，重点覆盖 `mysql`、`pdf-ingest-root`、`graphrag-root`、`graphrag-build-runs-root`、`graphrag-api`、`graphrag-ready` 和 `neo4j`。Neo4j 用于学生端知识图谱只读浏览，不可达时图谱页应显式降级。
