...
1. 管理用户与最小 RBAC 结构
2. 显式建模用户与课程之间的成员关系，并用它控制课程问答权限
3. 维持现有课程、PDF、解析结果、标准化导出与 GraphRAG 索引链路
4. 显式建模课程知识库与索引版本
5. 支撑问答会话、多轮消息、检索日志、命中文档追踪
6. 为后续接入更完整的课程平台能力留出清晰扩展口

## 2.2 明确不做

本次设计刻意不展开以下业务域：

1. 复杂的学期选课流程 / 班级 / 教学班 / 排课管理
2. 学习进度 / 课时完成率
3. 收藏 / 点赞 / 反馈 / 举报
4. 社区帖子 / 评论 / 关系网络
5. 把全文知识内容或向量数据强行存进 MySQL

其中“用户是否能访问某门课程”不再属于不做范围，而作为本版核心能力纳入设计；未纳入的是更复杂的教务流程和学习运营能力。
...
课程成员关系表，用于表达“哪个用户可以访问哪门课程”，并承载课程内角色。

推荐采用 `course_memberships` 而不是只做 `course_enrollments`，原因是首版虽然主要解决学生访问课程的问题，但后续教师、助教、课程管理员也可以复用同一张关系表。
...
- `membership_role enum('student','teacher','assistant')`
- `status enum('active','pending','suspended','removed')`
- `access_source enum('manual','imported','self_join','sync')`
- `source_ref_type varchar(64) nullable`
- `source_ref_id varchar(128) nullable`
- `joined_at timestamp nullable`
- `expires_at timestamp nullable`
- `effective_from timestamp nullable`
- `effective_to timestamp nullable`
- `granted_by_user_id bigint nullable fk -> users.id`
- `revoked_by_user_id bigint nullable fk -> users.id`
...
1. 对当前最小模型而言，普通学员在课程问答中的权限来源于 `course_memberships`
2. 首版推荐只有 `status='active'` 的成员才允许访问课程级知识库与问答
3. `membership_role` 是课程内身份，和平台全局 `roles` 是两层概念，不应混用
4. `admin` 全局角色可以作为平台级绕过权限，但普通 `user` 不能绕过课程成员关系直接访问任意课程
5. `course_memberships` 本身保存“当前有效授权快照”，不承担多版本历史主表职责，因此继续保留 `(user_id, course_id)` 唯一
6. `source_ref_type/source_ref_id` 用于为未来班级、教学班、学期选课或同步任务留出上游来源映射口

### `course_membership_events`

课程成员关系事件表，用于保存授权变更历史。该表采用追加写，不回收旧记录。

建议字段：

- `id bigint pk`
- `course_membership_id bigint fk -> course_memberships.id`
- `event_type enum('created','activated','suspended','removed','restored','expired')`
- `old_status varchar(32) nullable`
- `new_status varchar(32) nullable`
- `operator_user_id bigint nullable fk -> users.id`
...
1. `course_memberships` 负责“当前态”，`course_membership_events` 负责“历史轨迹”
2. 这种双层设计比直接把 `course_memberships` 做成多版本主表更稳，查询当前权限也更简单

### `courses` 的建议增量字段

为了明确课程访问策略，建议在现有 `courses` 表上增量增加以下字段：

- `status enum('active','inactive','archived')`
- `access_policy enum('restricted','public') default 'restricted'`

说明：

1. 首版课程默认 `access_policy='restricted'`
2. 当课程是 `restricted` 时，只有管理员或 `course_memberships.status='active'` 的用户才可访问课程级问答
3. 未来若需要公开试听课或开放课程，可通过 `public` 扩展，而不必重做权限模型
...
4. 首版不单独给 `knowledge_bases` 建访问控制表，默认继承所属 `course_id` 的课程访问权限
...
- `course_membership_id bigint nullable fk -> course_memberships.id`
...
3. `course_membership_id` 用于记录该课程会话是在什么课程成员授权之下创建的
4. 若两者同时存在，`knowledge_base_id` 对应的课程必须与 `course_id` 一致
5. 对普通用户，课程会话应要求存在有效的 `course_membership_id`；管理员可例外
...
访问授权审计日志表，用于记录课程问答权限判断结果，特别是管理员绕过与拒绝访问。

建议字段：

- `id bigint pk`
- `actor_user_id bigint fk -> users.id`
- `target_course_id varchar(64) nullable fk -> courses.course_id`
- `target_session_id bigint nullable fk -> qa_sessions.id`
- `course_membership_id bigint nullable fk -> course_memberships.id`
- `action varchar(64)`，例如 `create_course_session` / `open_course_session` / `query_course_kb`
- `decision enum('allow','deny')`
- `decision_reason varchar(64)`，例如 `active_membership` / `admin_override` / `no_membership` / `membership_expired` / `course_public`
...
1. 该表不替代 `qa_retrieval_logs`
2. `qa_retrieval_logs` 记录的是“进入检索后的执行事实”，而 `authorization_audit_logs` 记录的是“进入检索前的权限判定事实”
3. 最小可运行版至少要求记录两类事件：`admin_override` 与 `deny`
...
3. `users` 1 -> N `course_memberships`
4. `course_memberships` N -> 1 `courses`
5. `course_memberships` 1 -> N `course_membership_events`
6. `users` N -> N `roles`，通过 `user_roles`
7. `roles` N -> N `permissions`，通过 `role_permissions`
8. `users` 1 -> N `auth_identities`
9. `courses` 1 -> N `pdf_files`
10. `pdf_files` 1 -> N `parse_results`
11. `courses` 1 -> N `knowledge_bases`
12. `knowledge_bases` 1 -> N `kb_documents`
13. `knowledge_bases` 1 -> N `index_runs`
14. `index_runs` 1 -> N `index_artifacts`
15. `qa_sessions` N -> 1 `courses`（可空）
16. `qa_sessions` N -> 1 `course_memberships`（可空）
17. `qa_sessions` N -> 1 `knowledge_bases`（可空）
18. `qa_messages` 1 -> N `qa_retrieval_logs`
19. `qa_retrieval_logs` 1 -> N `qa_retrieval_hits`
20. `qa_retrieval_hits` N -> 1 `kb_documents`
21. `qa_retrieval_logs` N -> 1 `index_runs`
22. `users` 1 -> N `authorization_audit_logs`
...
2. `course_memberships` 的 `course_id` 与 `users`、`courses` 共同定义课程访问权限的事实来源
3. `qa_sessions` 若同时绑定课程、课程成员关系与知识库，则三者必须属于同一课程
4. 普通用户创建课程级 `qa_sessions` 时，必须拥有该课程的 `active course_membership`
5. `qa_retrieval_logs.assistant_message_id` 必须引用助手消息，而不是用户消息
6. `authorization_audit_logs` 必须覆盖至少两类高风险授权结果：`admin_override` 与 `deny`
...
1. `pending`：已建立关系，但尚未正式开放课程访问
2. `active`：可访问课程问答与课程知识库
3. `suspended`：临时停用，但保留历史关系
4. `removed`：移出课程，不再允许继续创建新的课程级问答
...
1. 前端发起问题
2. 若为课程级问答，先校验用户是否对目标 `course_id` 拥有 `active course_membership`，管理员可按全局角色放行
3. 在权限判定完成后写入 `authorization_audit_logs`
   - 普通用户合法访问时可按需抽样记录
   - `admin_override` 与 `deny` 必须强制记录
4. 创建或定位 `qa_sessions`
5. 插入用户消息到 `qa_messages`
6. 插入一条助手占位消息，状态为 `generating`
7. 根据会话上下文定位 `course_id`、`course_membership_id`、`knowledge_base_id` 与当前 `active_index_run_id`
8. 调用 GraphRAG 执行 `local` / `global` / `full` 查询
9. 写入 `qa_retrieval_logs`
10. 写入 `qa_retrieval_hits`
11. 更新助手消息内容、耗时与状态
12. 更新 `qa_sessions.last_message_at`

该时序的目的有两个：

1. 能完整还原一条对话历史
2. 能在出现错误时快速定位是“无课程权限”“无可用索引”“检索失败”“命中为空”还是“消息更新失败”
...
1. **可恢复错误**
   ...
   - 保留失败记录，允许用户重试
2. **业务阻断错误**
   - 例如用户没有课程访问权限，或课程没有可用 `active_index_run_id`
   - 直接写入明确错误提示，不进入检索
3. **一致性错误**
...
1. `course_memberships.membership_role`
   - 支持未来扩展学生、教师、助教等课程内身份
2. `qa_sessions.course_id` 可空
   - 允许平台级通用问答与课程内问答共存
3. `auth_identities`
   - 支持未来接入微信、OAuth、校园统一认证
4. `roles / permissions`
   - 支持未来扩展平台级管理员、运营等全局权限体系
5. `kb_documents.extra_metadata`
   - 允许后续扩展表格数、公式数、图片数、chunk 策略
6. `qa_messages.extra_metadata`
   - 允许后续扩展附件、模型参数、上下文窗口、审核信息
7. `course_memberships.source_ref_type/source_ref_id`
   - 支持未来把学期选课、教学班、同步任务等上游来源映射到当前有效访问授权
...
1. 课程成员关系变为 `active` 后，用户才能创建该课程的课程级问答
2. 无课程成员权限的普通用户不能访问不属于自己的课程
3. 管理员绕过课程权限访问时，必须落一条 `admin_override` 审计日志
4. 拒绝访问课程时，必须落一条 `deny` 审计日志
5. PDF 解析成功后能关联到知识库
6. 索引运行成功后能切换 `active_index_run_id`
7. 问答失败后仍保留用户消息与失败日志
...
以下三点在本设计中视为已收口的推荐决议，不再作为开放性遗留问题保留：

1. **`course_memberships` 采用“当前态单行 + 事件历史表”模型**
   - 主表继续保留 `(user_id, course_id)` 唯一
   - 历史变更通过 `course_membership_events` 追加记录
   - 不把 `course_memberships` 直接设计成多版本主表
2. **管理员绕过课程权限必须记录独立授权审计日志**
   - 使用 `authorization_audit_logs`
   - 最小可运行版至少强制记录 `admin_override` 与 `deny`
3. **未来班级 / 教学班 / 学期选课不直接替代 `course_memberships`**
   - `course_memberships` 始终表示“问答系统当前有效访问授权”
   - 教务侧的班级、教学班、学期选课等作为上游来源，通过 `source_ref_type/source_ref_id` 映射进来

### 13.2 风险与边界

本设计仍有以下已知边界：

1. 首版纳入了课程成员关系，但尚未扩展到复杂的班级、教学班、学期选课流程
2. `kb_documents` 保存的是“文档元数据 + 溯源锚点”，不是全文搜索库
3. 如果后续实际问答编排不再是“一次助手消息对应一次检索”，则需要扩展 `qa_retrieval_logs` 的编排字段
4. 若未来引入多租户或多校区隔离，还需在 `users`、`courses`、`knowledge_bases` 与 `course_memberships` 之上补充租户域
...
2. 用最小复杂度补齐用户、课程成员关系、知识库、索引版本、会话、消息、检索命中这些业务事实
3. 让系统能够明确回答：
   - 这是谁问的
   - 问的是哪门课
   - 他是否真的有这门课的访问权限
