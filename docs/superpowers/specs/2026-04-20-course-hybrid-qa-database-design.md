...
5. `users` N -> N `roles`，通过 `user_roles`
6. `roles` N -> N `permissions`，通过 `role_permissions`
7. `users` 1 -> N `auth_identities`
8. `courses` 1 -> N `pdf_files`
9. `pdf_files` 1 -> N `parse_results`
10. `courses` 1 -> N `knowledge_bases`
11. `knowledge_bases` 1 -> N `kb_documents`
12. `knowledge_bases` 1 -> N `index_runs`
13. `index_runs` 1 -> N `index_artifacts`
14. `qa_sessions` N -> 1 `courses`（可空）
15. `qa_sessions` N -> 1 `course_memberships`（可空）
16. `qa_sessions` N -> 1 `knowledge_bases`（可空）
17. `qa_messages` 1 -> N `qa_retrieval_logs`
18. `qa_retrieval_logs` 1 -> N `qa_retrieval_hits`
19. `qa_retrieval_hits` N -> 1 `kb_documents`
20. `qa_retrieval_logs` N -> 1 `index_runs`
...
3. 创建或定位 `qa_sessions`
4. 插入用户消息到 `qa_messages`
5. 插入一条助手占位消息，状态为 `generating`
6. 根据会话上下文定位 `course_id`、`course_membership_id`、`knowledge_base_id` 与当前 `active_index_run_id`
7. 调用 GraphRAG 执行 `local` / `global` / `full` 查询
8. 写入 `qa_retrieval_logs`
9. 写入 `qa_retrieval_hits`
10. 更新助手消息内容、耗时与状态
11. 更新 `qa_sessions.last_message_at`
...
3. PDF 解析成功后能关联到知识库
4. 索引运行成功后能切换 `active_index_run_id`
5. 问答失败后仍保留用户消息与失败日志
