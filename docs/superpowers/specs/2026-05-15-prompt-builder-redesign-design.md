...
- **顶部进度摘要**：横向 3 列，左为 "32 / 80 LLM 调用 · 已用 1 min · 预估剩余 < 1 min"（并发模式下所有候选同时推进），中为 "~ 192k tokens 已消耗 / 预估总量 480k"，右侧"中止评分"按钮。
- **候选矩阵**：每行一个候选，所有候选并发执行（同时蓝色高亮），列网格：候选名 / 抽取进度条 / 抽取状态 chip / 评分进度条 / 评分状态 chip / 完成后的综合分 chip。
  - 行级状态用色块表达：完成绿底（`success-soft`）、进行中蓝底（`running-soft`）。并发模式下不存在"排队灰显"状态。
