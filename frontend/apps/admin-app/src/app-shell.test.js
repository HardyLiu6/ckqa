...
test('Vite 构建把大型第三方依赖拆成稳定 vendor chunk', () => {
  ...
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/element-plus/es/index.mjs'), 'vendor-element-core')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/element-plus/es/components/button/index.mjs'), 'vendor-element-basic')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/element-plus/es/components/input/index.mjs'), 'vendor-element-forms')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/element-plus/es/components/table/index.mjs'), 'vendor-element-data')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/element-plus/es/components/dialog/index.mjs'), 'vendor-element-feedback')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/element-plus/es/components/menu/index.mjs'), 'vendor-element-navigation')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/element-plus/es/locale/lang/zh-cn.mjs'), 'vendor-element-locale')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/@element-plus/icons-vue/dist/index.mjs'), 'vendor-icons')
  assert.equal(resolveAdminAppManualChunk('/repo/node_modules/@element-plus/utils/index.mjs'), 'vendor-element-core')
...
test('知识库列表和详情 loader 映射实时字段与构建入口', async () => {
  ...
  const detailResult = await loadModulePage(
    ...
    {},
    {
      ...
      listIndexRuns: async () => [{ id: 12, status: 'success', knowledgeBaseId: 7, createdAt: '2026-04-28T10:00:00' }],
    },
  )

  assert.equal(detailResult.requestState, 'success')
  assert.equal(detailResult.blocks.knowledgeBase.item.id, 7)
  assert.equal(detailResult.blocks.indexRuns.items[0].to, '/app/index-runs/12?kbId=7')
...
test('索引详情面包屑通过 ?kbId query 回到知识库详情，缺失时降级为只到知识库列表', () => {
  // 完整 query → 父链：知识库 > 知识库列表 > 知识库详情 > 索引运行详情
  assert.deepEqual(
    buildConsoleBreadcrumbItems({
      name: 'index-run-detail',
      params: { indexRunId: '13' },
      query: { kbId: '4' },
      meta: { title: '索引运行详情', navGroup: 'knowledge' },
    }).map((item) => item.label),
    ['知识库构建', '知识库列表', '知识库详情', '索引运行详情'],
  )

  // 缺 kbId → 降级父链：知识库 > 知识库列表 > 索引运行详情
  assert.deepEqual(
    buildConsoleBreadcrumbItems({
      name: 'index-run-detail',
      params: { indexRunId: '13' },
...
test('知识库详情 loader 把索引运行按 build_run 分组，重试链上的失败索引归并到同一组', async () => {
  // 复刻 KB 4 真实数据：build_run 3 下有 #5 #6 两条 failed + 一条最终的 success（build_run 4 携带）
  // 期望：分组后 build_run 3 名下挂着 #6 #5（按 startedAt 倒序，重试链可见），
  //       build_run 13 / 12 / 11 各自独立成组
  const result = await loadModulePage(
    { name: 'knowledge-base-detail', query: {}, params: { kbId: '4' } },
    {},
    {
      getKnowledgeBase: async () => ({ id: 4, name: 'OS 知识库', courseId: 'os', activeIndexRunId: 13 }),
      listIndexRuns: async () => [
        { id: 13, buildRunId: 13, status: 'success', startedAt: '2026-05-18T20:02:00' },
        { id: 11, buildRunId: 12, status: 'success', startedAt: '2026-05-18T19:54:00' },
        { id: 8, buildRunId: 4, status: 'success', startedAt: '2026-05-05T20:19:00' },
        { id: 6, buildRunId: 3, status: 'failed', startedAt: '2026-05-05T20:14:00' },
        { id: 5, buildRunId: 3, status: 'failed', startedAt: '2026-05-05T20:12:00' },
        { id: 99, buildRunId: null, status: 'success', startedAt: '2026-05-01T10:00:00' },
      ],
      listKnowledgeBaseBuildRuns: async () => ({
        items: [
          { id: 13, buildVersion: 'kb4-002714192', status: 'success', createdAt: '2026-05-14T00:27:00' },
          { id: 12, buildVersion: 'kb4-001849456', status: 'success', createdAt: '2026-05-14T00:18:00' },
          { id: 4, buildVersion: 'kb4-201031374', status: 'success', createdAt: '2026-05-05T20:10:00' },
          { id: 3, buildVersion: 'kb4-201029334', status: 'success', createdAt: '2026-05-05T20:10:00' },
        ],
        current: 1,
        size: 200,
        total: 4,
  ...
  const groupsBlock = result.blocks.indexRunGroups
  assert.equal(groupsBlock.state, 'success')
  // 5 个组：4 个 build_run 各 1 组 + 1 个 orphan 组（buildRunId=null 的旧版索引）
  assert.equal(groupsBlock.items.length, 5)

  // build_run 3 那组应包含两条 failed 重试，按 startedAt 倒序（#6 在前）
  const group3 = groupsBlock.items.find((g) => g.buildRunId === 3)
  assert.ok(group3, 'build_run 3 应该有独立分组')
  assert.equal(group3.runs.length, 2, 'build_run 3 名下应有两条重试')
  assert.deepEqual(group3.runs.map((r) => r.id), [6, 5], '重试链按 startedAt 倒序')
  assert.equal(group3.buildVersion, 'kb4-201029334')
  assert.equal(group3.buildStatus, 'success')

  // 最新构建（build_run 13）应排在第一位
  assert.equal(groupsBlock.items[0].buildRunId, 13)

  // orphan 组（buildRunId=null）排在最末
  const orphanGroup = groupsBlock.items.at(-1)
  assert.equal(orphanGroup.buildRunId, null)
  assert.equal(orphanGroup.runs[0].id, 99)
  assert.match(orphanGroup.buildVersion, /未关联构建/)
})

test('知识库详情 loader 在 buildRuns API 缺失时仍能基于 indexRuns 分组', async () => {
  // 即使没传 listKnowledgeBaseBuildRuns，分组仍应工作（用 buildRunId 直接产出 group），
  // 仅元信息 buildVersion 退化为占位
  const result = await loadModulePage(
    { name: 'knowledge-base-detail', query: {}, params: { kbId: '4' } },
    {},
    {
      getKnowledgeBase: async () => ({ id: 4, courseId: 'os', activeIndexRunId: 13 }),
      listIndexRuns: async () => [
        { id: 13, buildRunId: 13, status: 'success', startedAt: '2026-05-18T20:02:00' },
      ],
      // 不传 listKnowledgeBaseBuildRuns
    },
  )
  const groupsBlock = result.blocks.indexRunGroups
  assert.equal(groupsBlock.items.length, 1)
  assert.equal(groupsBlock.items[0].buildRunId, 13)
  assert.equal(groupsBlock.items[0].buildVersion, '构建 #13')
})
