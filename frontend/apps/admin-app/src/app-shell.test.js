...
} from './composables/useQaPolling.js'
import {
  createCourse,
  deleteCourse,
  listCourseMaterials,
...
test('局部操作反馈按资料、索引和 QA 操作拆分标题与处理建议', () => {
  ...
  assert.equal(qaFeedback.title, '知识库验证失败')
...
test('问答会话列表页面模型保留正式问答和知识库验证过滤项', () => {
...
test('业务页列表筛选只按显式列匹配', () => {
  ...
  assert.deepEqual(
    filterRowsByFilters(qaSessions.rows, qaSessions.filters, { sessionType: '知识库验证' }).map((row) => row[0]),
    ['构建后抽样问答', '索引切换验证'],
  )
  assert.deepEqual(
...
test('创建表单使用 Element Plus 输入组件且顶部身份区保持只读', () => {
  ...
  assert.match(topbar, /class="app-topbar-cmd-trigger"/)
  assert.match(topbar, /identity-avatar/)
  ...
  assert.match(consoleLayout, /buildConsoleBreadcrumbItems\(/)
  assert.match(consoleLayout, /<CkBreadcrumbs[\s\S]*:items="breadcrumbItems"/)
...
test('操作按钮统一迁移到 Element Plus Button 并配置图标与高级态样式', () => {
  ...
  const healthPage = readFileSync(new URL('./views/system/HealthPage.vue', import.meta.url), 'utf8')
  const elementPlusCss = readFileSync(new URL('./styles/element-plus.scss', import.meta.url), 'utf8')
  const componentsCss = readFileSync(new URL('./styles/components.scss', import.meta.url), 'utf8')

  for (const source of [modulePage, tableShell, workflowStepper, topbar, loginView, routeState, unifiedErrorView, healthPage]) {
  ...
  assert.match(topbar, /class="app-topbar-avatar identity-avatar"/)
  ...
  assert.match(healthPage, /<el-button[\s\S]*class="ckqa-el-button ckqa-el-button--primary"/)
...
// ---------------------------------------------------------------------------
// P1 / P5 属性测试（对应 tasks.md 任务 1.5 起骨架、任务 7.5 阶段 7 收紧；
// 设计文档 §13.1 / §13.4 / §3.7）
//
// 阶段 7 收紧：所有 APP_ROUTES 条目的 componentKey 均不再指向 ModulePage。
// `ModulePage.vue` 文件本身仍保留为"可编译占位"兜底，不被任何路由直接引用；
// 本文件下方保留一条 import-level 冒烟测试，防止文件被误删但不再走功能路径。
//
// P5 快照常量 `APP_ROUTE_PATHS_SNAPSHOT` 是 M7 起点路径表的不变量，阶段 3~6
// 只允许改动 `componentKey / meta.status / meta.routeState / meta.keepAlive`，
// **任何 path 改动都必须经过明确评审**并同步更新此快照（同时在 PR 描述
// 里说明原因）。
// ---------------------------------------------------------------------------
...
// 解析 `src/router/index.js` 中 `componentMap` 对象里声明的组件 key。
// 采用文本解析而非直接 import，是因为 `node --test` 加载不了 `.vue` 文件。
function parseComponentMapKeys() {
  const source = readFileSync(
    new URL('./router/index.js', import.meta.url),
    'utf8',
  )
  const match = source.match(/const\s+componentMap\s*=\s*\{([\s\S]*?)\}/)
  assert.ok(match, 'router/index.js 未找到 componentMap 定义')
  return match[1]
    .split('\n')
    .map((line) => line.replace(/\/\/.*$/, '').trim().replace(/,$/, ''))
    .filter((line) => line.length > 0 && !line.startsWith('//'))
    .map((line) => {
      // 支持 `Foo` 简写与 `Foo: Bar` 长写两种对象属性声明
...
test('P1：APP_ROUTES 所有 componentKey 可解析到组件字典，且一律不得为 ModulePage', () => {
  const componentKeys = new Set(parseComponentMapKeys())

  fc.assert(
    fc.property(fc.constantFrom(...APP_ROUTES), (route) => {
      if (typeof route.componentKey !== 'string') {
        // 允许纯 redirect 条目（如 `/app` / `/app/system`）没有 componentKey
        return route.redirect !== undefined
      }

      assert.ok(
        componentKeys.has(route.componentKey),
        `路由 ${route.name ?? route.path} 的 componentKey "${route.componentKey}" 未在 router/index.js 的 componentMap 中声明`,
      )

      assert.notStrictEqual(
        route.componentKey,
        'ModulePage',
        `路由 ${route.name ?? route.path} 不应再直接指向 ModulePage（阶段 7 收紧：所有 APP_ROUTES 条目的 componentKey 均不得指向 ModulePage）`,
      )
      return true
    }),
    { numRuns: 50 },
...
// ModulePage 可编译（import-level 冒烟）：
// 在 `node --test` 运行环境下无法直接 import .vue 文件，因此沿用本文件对
// `HealthPage.vue` 等 SFC 的做法，用 readFileSync + existsSync 断言文件存在
// 且内容非空（> 1000 字节表示仍是真实占位，不是被清空的空文件）。
// 该断言的目的：阶段 7 收紧 P1 后 ModulePage 已不走功能路径，但文件仍保留
// 作为可回滚兜底；此测试防止文件被误删/误清空。
test('ModulePage 可编译占位：文件仍存在且非空（import-level 冒烟测试）', () => {
  const modulePageUrl = new URL('./views/pages/ModulePage.vue', import.meta.url)
  assert.ok(existsSync(modulePageUrl), 'ModulePage.vue 文件缺失：阶段 7 后仍需保留为回滚兜底占位')

  const modulePageSource = readFileSync(modulePageUrl, 'utf8')
  assert.ok(
    modulePageSource.length > 1000,
    `ModulePage.vue 内容过短（${modulePageSource.length} 字节），疑似被清空；阶段 7 后仍需保留原始实现以便回滚兜底`,
  )
})

// P5：APP_ROUTES path 不变快照。
// 此快照是 M7 内的不变量，阶段 3~6 的路由替换只允许改 componentKey / meta，
// 任何 path 增删改都必须先在 PR 评审中明确批准并同步更新此常量。
const APP_ROUTE_PATHS_SNAPSHOT = [
  '/app',
  ...
  '/app/dashboard',
  '/app/health',
  '/app/index-runs/:indexRunId',
  '/app/knowledge-bases',
  '/app/knowledge-bases/:kbId',
...
test('P5：APP_ROUTES 的 path 集合与 M7 起点快照一致', () => {
  const actual = APP_ROUTES.map((route) => route.path).sort()
  assert.deepEqual(
    actual,
    APP_ROUTE_PATHS_SNAPSHOT,
    'APP_ROUTES 的 path 发生了变动。如确需改动，请同步更新 APP_ROUTE_PATHS_SNAPSHOT 并在 PR 描述中说明原因。',
  )
})
