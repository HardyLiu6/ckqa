...
test('composer 底部模式提示优先显示当前运行任务的真实模式', () => {
  assert.match(qaPageSource, /const composerModeHintLabel = computed/)
  assert.match(qaPageSource, /hasActivePendingTask\.value[\s\S]*messageModeLabel\(pendingTask\.value\)/)
  const composerHintBlock = qaPageSource.match(/<div v-if="!isEmpty" class="composer-hint">[\s\S]*?<\/div>/)?.[0] ?? ''
  assert.match(composerHintBlock, /\{\{ composerModeHintLabel \}\}/)
  assert.doesNotMatch(composerHintBlock, /getModeOption\(modePreview\.mode\)\.shortLabel/)
})
