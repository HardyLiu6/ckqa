...
test('drift 追问扩展显示追问线索总数和可用依据数量', () => {
  const timeline = buildRetrievalTimeline([
    {
      type: 'retrieval_started',
      mode: 'drift',
    ...
    },
    {
      type: 'reduce_started',
      mode: 'drift',
      summary: '已生成 8 条追问线索，完成 1 条可用依据，正在汇总回答。',
      metrics: {
        driftNodeCount: 8,
        answeredNodeCount: 1,
        pendingNodeCount: 7,
  ...
  assert.equal(timeline.items[1].title, '追问扩展')
  assert.equal(timeline.items[1].evidenceLabel, '可用依据 1 / 追问线索 8 条')
  assert.equal(timeline.items[1].summary, '已生成 8 条追问线索，其中 1 条形成可用依据。')
})
