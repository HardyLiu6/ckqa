...
test('会话统计解析后端聚合的真实口径', () => {
  assert.deepEqual(
    normalizeQaSessionStats({ totalSessions: 128, totalMessages: 940, courseCount: 6, favoriteCount: 18 }),
    { totalSessions: 128, totalMessages: 940, courseCount: 6, favoriteCount: 18 },
  )
})
...
test('会话统计对缺失/异常字段安全兜底为 0', () => {
  assert.deepEqual(
    ...
    { totalSessions: 0, totalMessages: 0, courseCount: 0, favoriteCount: 0 },
  )
  assert.deepEqual(
    normalizeQaSessionStats({ totalSessions: '940', totalMessages: -3, courseCount: null, favoriteCount: -1 }),
    { totalSessions: 940, totalMessages: 0, courseCount: 0, favoriteCount: 0 },
  )
})
