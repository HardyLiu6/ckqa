...
事件包括 `ack/status/progress/delta/sources/done/error/heartbeat`，带序号的事件可被 Java 用于 `afterEventSeq` 断线恢复。如果 GraphRAG 原生 streaming 不可用、artifact 缺失或连接中断，Java 会继续使用阶段 1 的状态流和最终答案分段，不影响 MySQL 中的最终 assistant message 与 sources。
