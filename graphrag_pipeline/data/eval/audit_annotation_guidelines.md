...
- `FormulaOrDefinition`：公式或定义。具有稳定名称的定义、公式、定律、判定条件或符号化表达，用于支撑定义问答与公式检索。
- `AlgorithmOrMethod`：算法或方法。具有明确步骤、策略或机制的算法、方法、协议、处理流程或实现策略。
- `Experiment`：实验。课程中的实验、上机、课程项目、综合实践或验证任务。
- `Assignment`：作业。作业、习题、报告、测验、考试题组或用于评估学习结果的任务集合。
...
- `belongs_to`：属于。表示实体从属于某个课程结构单元或教学容器，是 contains 的反向表达。
...
- `evaluated_by`：由…评估。表示课程目标、章节、知识点、方法或实验由作业、测验或实验任务进行评估与验证。
- `appears_in`：出现于。表示实体在某章、某节、某实验或某作业中出现，是弱定位关系。
