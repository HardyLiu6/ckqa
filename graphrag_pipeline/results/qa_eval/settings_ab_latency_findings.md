...
2026-05-13 使用重新索引产物 `output/auto_tuned_crs-20260506-r4slkr_material7_20260513_001047` 复测：

| 对比项 | 结果 |
| --- | ---: |
| `community_reports.parquet` 行数 | 930 |
| `full_content` 空值数 | 0 |
| `lancedb/community_full_content.lance` | 存在 |
| drift 轻量配置，单题 API smoke | 成功，116.2723s |
| drift 一轮 follow-up，单题 API smoke | 360s 超时 |

结论：新索引已经修复旧产物缺少 `community_full_content` 向量表导致的 DRIFT 不可返回问题；但官方 DRIFT 一轮 follow-up 仍不适合同步 baseline。当前默认保留 `drift_k_followups=1`、`n_depth=0` 的轻量档，用于让 drift 可以在 360s 内稳定落盘。
...
- 新索引 drift 一轮 follow-up 探针：`results/qa_eval/runs/baseline-newindex-drift-smoke-20260513-1230`
- 新索引 drift 轻量探针：`results/qa_eval/runs/baseline-newindex-drift-lite-20260513-1236`
...
3. `drift_search` 在 GraphRAG 3.0.9 中的主要慢点不是单纯的 local follow-up，而是 primer 与 community full-content 检索链路。旧索引下即使 `drift_k_followups=1`、`n_depth=0`，90s 内仍未返回；新索引补齐 `community_full_content.lance` 后，轻量配置可在 116.2723s 返回。
4. 源码检查显示 `drift_search.data_max_tokens`、`primer_llm_max_tokens` 这类配置字段在当前 DRIFT 查询实现中没有实质接入，因此继续只调 settings 对完整 DRIFT follow-up 的收益有限。

## 下一步建议

1. 保留当前 B 组 `global_search` 配置，继续用更多题型验证质量是否下降。
2. `drift` 可参与 smoke 或少量抽样，但同步全量 baseline 应使用更长超时或异步任务方式；一轮 follow-up 当前仍会拖垮总耗时。
3. 若必须优化完整 drift，需要进入代码/prompt 层：
