...
""",
    ...
    def test_directional_and_distilled_candidates_avoid_full_audit_text_and_record_source_policy(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("source 是被考核或评估的知识、概念、术语或方法", directional_text)
            ...
            self.assertIn("不要输出 Assignment -> Concept/Term 的 evaluated_by", distilled_v2_text)
            self.assertIn("正确方向：Concept/Term/KnowledgePoint/AlgorithmOrMethod -> Assignment", distilled_v2_text)
            self.assertIn("不要输出 Section/Assignment -> Concept 的 appears_in", distilled_v2_text)
            self.assertIn("位置容器或平台上下文", distilled_v2_text)
