...
def test_v6_selector_skips_dense_for_broad_chapter_summary():
    rows = [
        TextUnitRow("chapter11111", "第五章 虚拟存储器 本章介绍虚拟存储器的基本概念。"),
        TextUnitRow("gold22222222", "5.2 请求分页存储管理方式 请求分页系统支持虚拟存储器功能。"),
        TextUnitRow("other3333333", "进程调度负责在就绪进程之间分配处理机。"),
    ]
    encoder_calls: list[list[str]] = []

    def fake_encoder(texts: list[str]) -> np.ndarray:
        encoder_calls.append(texts)
        return np.asarray([[0.0, 1.0], [0.0, 1.0], [1.0, 0.0], [1.0, 0.0]], dtype=np.float32)

    selector = build_v6_hybrid_evidence_selector_from_rows(
        rows,
        config=V6EvidenceSelectorConfig(
            user_terms=("虚拟存储器", "请求分页"),
            enable_dense_rerank=True,
            dense_encoder=fake_encoder,
    ...
    refs = [candidate.ref for candidate in selector.search("请概括第五章「虚拟存储器」的核心内容。", top_k=2)]

    assert "gold22222222" in refs
    assert encoder_calls == []


def test_v6_selector_keeps_dense_for_specific_section_summary():
    rows = [
        TextUnitRow("lexical11111", "第一章 1.4 操作系统的主要功能。"),
        TextUnitRow("semantic2222", "处理机管理、存储器管理、设备管理和文件管理是操作系统的主要功能。"),
        TextUnitRow("other3333333", "银行家算法用于避免死锁。"),
    ]
    encoder_calls: list[list[str]] = []
    ...
    def fake_encoder(texts: list[str]) -> np.ndarray:
        ...
        vectors = []
        for text in texts:
            if "学习重点" in text or ("主要功能" in text and "管理" in text):
                vectors.append([1.0, 0.0])
            else:
                vectors.append([0.0, 1.0])
        return np.asarray(vectors, dtype=np.float32)
    ...
    selector = build_v6_hybrid_evidence_selector_from_rows(
        rows,
        config=V6EvidenceSelectorConfig(
            user_terms=("操作系统", "主要功能", "处理机管理", "存储器管理"),
            enable_dense_rerank=True,
            dense_encoder=fake_encoder,
            dense_rerank_candidate_pool_k=3,
        ),
    ...
    refs = [candidate.ref for candidate in selector.search("请概括第一章 1.4「操作系统的主要功能」的学习重点。", top_k=2)]

    assert refs[0] == "semantic2222"
    assert len(encoder_calls) == 1
