...
def _write_core_index_output(path: Path) -> None:
    for relative_path in CORE_INDEX_OUTPUT_FILES:
...
def _write_three_item_test_set(path: Path) -> None:
    payloads = []
        ...
        payloads.append(
            {
                ...
                "category": "factual_lookup",
                "question": f"第 {index} 个问题是什么？",
                "gold_answer_summary": f"第 {index} 个参考答案",
                "gold_entities": [f"实体{index}"],
                "gold_text_unit_ids": ["d244f9016ac84a55a7435cb6"],
                "must_cite_terms": [],
                "negative_terms": [],
            }
...
def test_runner_records_validated_index_output_dir_in_run_meta(tmp_path, monkeypatch):
    from graphrag_pipeline.scripts.qa_eval import run_baseline_eval
    from graphrag_pipeline.scripts.qa_eval.run_baseline_eval import BaselineEvalRunner
    ...
    meta = json.loads((run_dir / "run_meta.json").read_text(encoding="utf-8"))
    assert meta["index_output_dir"] == str(index_output_dir)


def test_runner_persists_relative_index_output_dir_as_absolute_path(tmp_path, monkeypatch):
    from graphrag_pipeline.scripts.qa_eval import run_baseline_eval
    from graphrag_pipeline.scripts.qa_eval.run_baseline_eval import BaselineEvalRunner

    test_set = tmp_path / "test_set.jsonl"
    relative_index_dir = Path("output") / "auto_tuned_index"
    absolute_index_dir = tmp_path / relative_index_dir
    _write_single_item_test_set(test_set)
    _write_core_index_output(absolute_index_dir)
    ...
    run_dir = BaselineEvalRunner(
        test_set_path=test_set,
        output_root=tmp_path / "runs",
        run_id="run-relative-index-dir",
        index_run_label="index-A",
        index_output_dir=relative_index_dir,
        modes=["local"],
    ).run()

    meta = json.loads((run_dir / "run_meta.json").read_text(encoding="utf-8"))
    assert meta["index_output_dir"] == str(absolute_index_dir)


def test_runner_rejects_index_output_dir_missing_core_files(tmp_path, monkeypatch):
    from graphrag_pipeline.scripts.qa_eval import run_baseline_eval
    from graphrag_pipeline.scripts.qa_eval.run_baseline_eval import BaselineEvalRunner

    test_set = tmp_path / "test_set.jsonl"
    index_output_dir = tmp_path / "output" / "incomplete_index"
    _write_single_item_test_set(test_set)
    _write_core_index_output(index_output_dir)
    (index_output_dir / "text_units.parquet").unlink()
    (index_output_dir / "lancedb" / "text_unit_text.lance").unlink()
    monkeypatch.setattr(
        run_baseline_eval,
        "validate_jsonl",
        lambda path: ValidationReport(ok=True, total=1),
    ...
    runner = BaselineEvalRunner(
        test_set_path=test_set,
        output_root=tmp_path / "runs",
        run_id="bad-index-dir",
        index_run_label="index-A",
        index_output_dir=index_output_dir,
    )
    with pytest.raises(FileNotFoundError, match="text_units.parquet.*lancedb/text_unit_text.lance"):
        runner.run()
...
def test_runner_can_execute_selected_question_ids_in_requested_order(tmp_path, monkeypatch):
    from graphrag_pipeline.scripts.qa_eval import run_baseline_eval
    from graphrag_pipeline.scripts.qa_eval.run_baseline_eval import BaselineEvalRunner

    test_set = tmp_path / "test_set.jsonl"
    _write_three_item_test_set(test_set)
    validate_calls: list[Path] = []

    def _fake_validate(path: Path) -> ValidationReport:
        validate_calls.append(path)
        return ValidationReport(ok=True, total=3)

    monkeypatch.setattr(run_baseline_eval, "validate_jsonl", _fake_validate)
    _FakeClient.calls = []
    _FakeClient.failures = set()
    monkeypatch.setattr(run_baseline_eval, "OpenAICompatibleClient", _FakeClient)
    ...
    meta = json.loads((run_dir / "run_meta.json").read_text(encoding="utf-8"))
    assert validate_calls == [test_set]
    assert meta["question_ids"] == ["Q003", "Q001"]
    assert meta["total_items"] == 2
    assert meta["total_questions"] == 2
    assert meta["source_total_questions"] == 3
    assert sorted(path.name for path in (run_dir / "raw").glob("Q*.json")) == ["Q001.json", "Q003.json"]
    assert [prompt for _, prompt in _FakeClient.calls] == ["第 3 个问题是什么？", "第 1 个问题是什么？"]


def test_runner_records_executed_question_ids_after_max_items(tmp_path, monkeypatch):
    from graphrag_pipeline.scripts.qa_eval import run_baseline_eval
    from graphrag_pipeline.scripts.qa_eval.run_baseline_eval import BaselineEvalRunner
    ...
    meta = json.loads((run_dir / "run_meta.json").read_text(encoding="utf-8"))
    assert meta["question_ids"] == ["Q003"]
    assert meta["total_items"] == 1
    assert meta["total_questions"] == 1
    assert sorted(path.name for path in (run_dir / "raw").glob("Q*.json")) == ["Q003.json"]
    assert [prompt for _, prompt in _FakeClient.calls] == ["第 3 个问题是什么？"]


def test_runner_rejects_unknown_question_ids(tmp_path, monkeypatch):
    from graphrag_pipeline.scripts.qa_eval import run_baseline_eval
    from graphrag_pipeline.scripts.qa_eval.run_baseline_eval import BaselineEvalRunner
    ...
    runner = BaselineEvalRunner(
        test_set_path=test_set,
        output_root=tmp_path / "runs",
        run_id="bad-question-ids",
        index_run_label="index-A",
        question_ids=["Q001", "Q999"],
    )
    with pytest.raises(ValueError, match=r"unknown question id\(s\): Q999"):
        runner.run()


def test_runner_rejects_duplicate_question_ids(tmp_path, monkeypatch):
    from graphrag_pipeline.scripts.qa_eval import run_baseline_eval
    from graphrag_pipeline.scripts.qa_eval.run_baseline_eval import BaselineEvalRunner
    ...
    runner = BaselineEvalRunner(
        test_set_path=test_set,
        output_root=tmp_path / "runs",
        run_id="duplicate-question-ids",
        index_run_label="index-A",
        question_ids=["Q001", "Q001"],
    )
    with pytest.raises(ValueError, match=r"duplicate question id\(s\): Q001"):
        runner.run()
...
def test_cli_main_wires_args_and_writes_output(tmp_path, monkeypatch):
    ...
    test_set = tmp_path / "test_set.jsonl"
    output_root = tmp_path / "runs"
    index_output_dir = tmp_path / "output" / "cli_index"
    _write_single_item_test_set(test_set)
    _write_core_index_output(index_output_dir)
    monkeypatch.setattr(
        run_baseline_eval,
