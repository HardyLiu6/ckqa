...
> 一期基线：课程资料模型、MinerU 解析、页级进度、标准化导出与下游 GraphRAG 契约均已落地。这里是稳定的知识生产入口；后续修改应优先保持导出兼容性，而非重新定义主流程。
...
SELECT r.role_code, r.role_name, COUNT(ur.user_id) AS user_count
FROM roles r
