...
```bash
python scripts/pdf_processor/mineru_parser.py parse <course_id>
```
...
```bash
python scripts/pdf_processor/mineru_parser.py status <course_id>
```
...
```bash
python scripts/pdf_processor/mineru_parser.py download <course_id> -o <output_dir>
```
...
解析失败后状态会变为 `failed`，可以直接再次运行 `parse` 命令重试。
