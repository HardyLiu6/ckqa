...
class DatabaseService:
    ...
    def check_md5_exists(self, file_md5: str) -> Optional[Dict]:
        ...
        Returns:
            如果存在返回文件记录，否则返回None
        """
        return self.get_pdf_file_by_md5(file_md5)
    ...
    def update_parse_status(self, file_id: int, status: ParseStatus,
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(f"""
                UPDATE pdf_files SET {', '.join(updates)} WHERE id = %s
            """, params)
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute("""
                INSERT INTO parse_results 
                    (pdf_file_id, course_id, result_type, file_name, 
    ...
    def get_parse_results(self, pdf_file_id: int) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM parse_results WHERE pdf_file_id = %s",
                (pdf_file_id,)
            )
            return cursor.fetchall()
    ...
    def delete_parse_results(self, pdf_file_id: int):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM parse_results WHERE pdf_file_id = %s",
                (pdf_file_id,)
            )
    ...
    def add_log(self, pdf_file_id: int, message: str, 
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO parse_logs (pdf_file_id, log_level, log_message)
                VALUES (%s, %s, %s)
    ...
    def get_logs(self, pdf_file_id: int) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM parse_logs WHERE pdf_file_id = %s ORDER BY created_at",
                (pdf_file_id,)
            )
            return cursor.fetchall()
    ...
    def get_pending_files(self) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM pdf_files WHERE parse_status = 'pending'"
            )
            return cursor.fetchall()
    ...
    def get_stats(self) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            # 总文件数
            cursor.execute("SELECT COUNT(*) as count FROM pdf_files")
            stats["total_files"] = cursor.fetchone()["count"]
            
            # 各状态文件数
            cursor.execute("""
                SELECT parse_status, COUNT(*) as count 
                FROM pdf_files GROUP BY parse_status
            """)
            stats["status_counts"] = {
